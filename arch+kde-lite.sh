...
configure() {
    clear
    echo 'Setting hostname ex: toshibaC840'
    set_hostname 

    echo 'Setting timezone ex: Asia/Jakarta'
    set_timezone 
    ...
    echo 'Installing KDE-Desktop minimal'
	set_kde
	
	clear
	echo 'Installing APPS: browser, office, media player dll'
	set_aplikasi_tambahan
...
install_packages() {
	...
	# Xserver
	packages+=' xorg xorg-server xorg-xinit xdg-user-dirs'
...
set_kde() { 
	clear
	install_kde
	echo 'activate services and daemon'
}
...
install_kde() {
	...
	#utility
	packages+=' power-profiles-daemon upower polkit python-gobject meson python-dbusmock python-isort python-mccabe umockdev'
...
set_aplikasi_tambahan() {
	local packages=''
	#apps
	packages+=' firefox libreoffice-still elisa vlc geany htop'
	pacman -Sy --noconfirm $packages
}
...
user_root() {
	...
	echo `systemctl enable power-profiles-daemon`
}
user_biasa() {
	echo `sudo systemctl enable sddm`
	echo `sudo systemctl enable NetworkManager`
	echo `sudo systemctl enable upower`
